        ...
        if not doctor.user:
            ...
            with transaction.atomic():
                ...
                if (
                    ...
                    # Get all existing awards associated with the doctor
                    ...
                    existing_appointments = AppointmentSchedule.objects.filter(doctor_profile=doctor)
                    print(existing_appointments)
                    # Iterate through the existing awards
                    for award_instance, award, honor, publication, research_interest, education_instance, certificate_degree, institution, board_id, result, passing_year, off_day_instance, off_day, schedule_time_instance, start_time, end_time, appointment_instance in zip(
                            ...
                            off_days, existing_schedule_times, start_times, end_times, existing_appointments
                    ):
                        try:
                            board = Board.objects.get(id=board_id)
                            off_day_id = OffDay.objects.get(id=off_day)
                        except (Board, OffDay).DoesNotExist as e:
                            data = {'status': 404, 'message': 'Board or off day are not Existing in db', 'error': e}
                        ...
                        off_day_serializer = OffDaySerializer(
                            off_day_instance, data={
                                'off_day': off_day_id.id,
                                'doctor_profile': doctor.id
                            })
                        print(schedule_time_instance)
                        schedule_time_serializer = ScheduleTimeSerializer(
                            schedule_time_instance, data={
                                'start_time': start_time,
                                'end_time': end_time,
                                # 'appointment_schedule': appointment_instance.id,
                                'doctor_profile': doctor.id
                            },
                        )
                        print(schedule_time_serializer.is_valid())
                        if (
                            ...
                            schedule = schedule_time_serializer.save(updated_at=timezone.now())
                        else:
                            transaction.set_rollback(True)
                            return Response(
                                {'status': 404, 'message': 'Request Data are invalid.'})
                    if award and edu and day and schedule:
                        return Response({'status': 200})
                    else:
                        transaction.set_rollback(True)
                        return Response(
